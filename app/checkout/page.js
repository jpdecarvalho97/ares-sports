...
export const dynamic = 'force-dynamic'; // evita erro de prerender na Vercel
...
function CheckoutInner(){
  ...
  const total = (cart?.total?.() || 0).toFixed(2);
  ...
  async function gerarPix(e){
    ...
    const res = await fetch("/api/pix", {
      method:"POST",
      body: JSON.stringify({ ...payload, amount: cart?.total?.() || 0 })
    });
      ...
      {step==="pix" && pix && (
        <div className="row row-2" style={{marginTop:12}}>
          <div>
            ...
            {/* Se usar Pagar.me depois, troque para pix.qr_code_image (url/base64) */}
